...
class DssmLstmPredicter:
    def __init__(self):
        ...
        with self.graph.as_default():
            with tf.variable_scope('dssm_lstm_query'):
                self.model = LSTMDSSM(self.vocab_file, self.num_lstm_units, self.vocab_size)
    ...
    def predict(self, input_text, question_standard):
        ...
        pr, no = self.session.run(
            ...
            feed_dict={self.model.queries: p,
                       self.model.docs: h,
    ...
    def predict_batch_test(self, input_text, question_standard, batch_size):
        ...
        for i in range(batch_count):
            ...
            prediction, prob = self.session.run(
                [self.model.prediction, self.model.prob],
                feed_dict={self.model.queries: input_vector,
                           self.model.docs: input_vector2,
                           self.model.sequence: self.io_sequence_size,
                           self.model.keep_prob: 1
                           })
            result.extend(prob.tolist())
            # for j in range(len(prob.tolist())):
            #     if j % 2 == 0:
            #         result.append(prob.tolist()[j])
            # return prob.tolist()
        return result

    def convert_batch(self, xitems, yitems, batch_size):
        xrecords = np.zeros((batch_size, self.io_sequence_size))
        yrecords = np.zeros((batch_size, self.io_sequence_size))
        for i in range(len(xitems)):
            count = len(xitems[i])
            if count > self.io_sequence_size:
                count = self.io_sequence_size
            count2 = len(yitems[i])
            if count2 > self.io_sequence_size:
                count2 = self.io_sequence_size
...
if __name__ == "__main__":
    MODLE_ = DssmLstmPredicter()
    MODLE_.predict('data1', 'data2')
